...
exports.addCourse = asyncHandler(async (req, res, next) => {
  ...
  if (
    ...
    return next(
      new ErrorResponse(
        `User ${req.user.email} is not authorized to add a course to bootcamp ${bootcamp.name}`,
        401
      )
    );
  }
...
exports.updateCourse = asyncHandler(
  async (req, res, next) => {
    ...
    if (
      ...
      return next(
        new ErrorResponse(
          `User ${req.user.email} is not authorized to update course ${course.title}`,
          401
        )
      );
    }
...
exports.deleteCourse = asyncHandler(
  async (req, res, next) => {
    ...
    if (
      ...
      return next(
        new ErrorResponse(
          `User ${req.user.email} is not authorized to delete course ${course.title}`,
          401
        )
      );
    }
