...
exports.createBootcamp = asyncHandler(async (req, res, next) => {
  ...
  if (publishedBootcamp && req.user.role !== 'admin') {
    return next(
      new ErrorResponse(
        `The user ${req.user.email} has already published a bootcamp`,
        400
      )
    );
  }
...
exports.bootcampPhotoUpload = asyncHandler(async (req, res, next) => {
  ...
  if (bootcamp.user.toString() !== req.user.id && req.user.role !== 'admin') {
    return next(
      new ErrorResponse(
        `User ${req.user.email} is not authorized to update this bootcamp photo`,
        404
      )
    );
  }
  ...
  file.mv(`${process.env.FILE_UPLOAD_PATH}/${file.name}`, async (error) => {
    ...
    res.status(200).json({
      success: true,
      data: file.name,
      path: process.env.FILE_UPLOAD_PATH + '/' + file.name,
    });
  });
});
